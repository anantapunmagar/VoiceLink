    ...
    <div className="relative px-2 py-2 flex-shrink-0 bg-[color:var(--color-bg-1)]">
      ...
      {showStatus && (
        <div className="absolute bottom-full left-2 right-2 mb-1.5 rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-bg-4)] shadow-2xl p-1.5 z-50 animate-slide-up">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-[color:var(--color-text-mute)] px-2 py-1">
            Set Status
          </p>
          {STATUS_OPTS.map((s) => (
            <button
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex items-center gap-2 flex-1 min-w-0 rounded-xl p-1.5 hover:bg-[color:var(--color-bg-4)] transition-colors focus-ring"
        >
          <Avatar user={user} size="sm" showStatus />
          <div className="flex-1 min-w-0 text-left">
            <p className="text-xs font-semibold text-[color:var(--color-text)] truncate leading-tight">
              {user.username}
            </p>
