      ...
      <div className="overflow-y-auto max-h-[80dvh] p-5">
        ...
        <div className="rounded-xl overflow-hidden border border-[color:var(--color-border)] mb-5">
          ...
          <div className="flex items-end gap-3 px-4 pb-3 -mt-7 bg-[color:var(--color-bg-4)]">
            <div
              className="h-14 w-14 rounded-full border-4 border-[color:var(--color-bg-4)] overflow-hidden flex items-center justify-center flex-shrink-0"
              style={{ background: avatar ? 'transparent' : colorForName(preview) }}
            >
              {avatar ? (
                <img src={avatar} alt="avatar" className="h-full w-full object-cover" />
              ) : (
                <span className="text-lg font-bold text-white">{initials(preview)}</span>
              )}
            </div>
            <div className="pb-1">
              <p className="font-semibold text-[color:var(--color-text)] text-sm">{preview}</p>
              {customStatus && (
                <p className="text-xs text-[color:var(--color-text-dim)]">{customStatus}</p>
              )}
            </div>
          </div>
          {bio && (
            <p className="px-4 pb-3 text-xs text-[color:var(--color-text-dim)] bg-[color:var(--color-bg-4)] leading-relaxed">
              {bio}
            </p>
          )}
        </div>
        ...
        <div className="space-y-5">
          {/* Avatar */}
          <div>
            <label className="text-[10px] font-semibold uppercase tracking-wider text-[color:var(--color-text-mute)] block mb-2">
              Profile Picture
            </label>
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <p className="text-[10px] text-[color:var(--color-text-mute)]">
                  JPG, PNG, GIF &middot; Max 3 MB
                </p>
              </div>
              <input
                ref={fileRef}
              ...
              {BANNERS.map((c) => (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: banner === c ? '0 0 0 2px var(--color-bg-3)' : 'none',
                  }}
                />
              ))}
            </div>
          ...
          <Input
            ...
            maxLength={24}
            hint="3-24 chars. Letters, numbers, underscores, dots, dashes."
          />
          <div>
            <Textarea
              label="About Me"
              ...
              maxLength={200}
            />
            <p className="text-[10px] text-[color:var(--color-text-mute)] mt-1 text-right">
              {bio.length}/200
            </p>
          </div>

          {error && (
            <p className="text-sm text-[color:var(--color-danger)] bg-[color:var(--color-danger-soft)] px-3 py-2 rounded-lg">
              {error}
            </p>
          )}
        </div>
