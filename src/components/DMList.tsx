...
export function DMList({ currentUser, selectedPeerId, onSelectPeer, onClose }: DMListProps) {
  ...
  useEffect(() => {
    function refresh() {
      ...
      dms
        .filter((d) => d.recipientId === currentUser.id && !d.read)
        .forEach((d) => {
  ...
  const othersFiltered = allUsers.filter(
    (u) => !interacted.has(u.id) && u.username.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="flex flex-col h-full bg-transparent">
      <div className="px-3 py-3 border-b border-[color:var(--color-border)] flex-shrink-0">
        <h2 className="text-sm font-semibold text-[color:var(--color-text)] mb-3">
          Direct Messages
        </h2>
        <button
          onClick={() => alert('Friend requests coming soon!')}
          className="w-full flex items-center justify-between px-2 py-2 mb-3 rounded-lg bg-[color:var(--color-bg-3)] hover:bg-[color:var(--color-bg-4)] text-[color:var(--color-text)] transition-colors group"
        >
          ...
          <span className="text-xs text-[color:var(--color-text-mute)] group-hover:text-[color:var(--color-text)] px-1">
            NEW
          </span>
        </button>
        <div className="relative">
          <Search
            size={13}
            className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[color:var(--color-text-mute)]"
          />
          <input
            value={search}
      ...
      <div className="flex-1 overflow-y-auto py-2 px-2">
        ...
        {sortedFiltered.map((peer) => {
          const lastDM = storage
            .getDMs()
            .filter(
              (d) =>
                (d.authorId === peer.id && d.recipientId === currentUser.id) ||
                (d.authorId === currentUser.id && d.recipientId === peer.id)
            )
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-1">
                  <span
                    className={cn(
                      'text-sm truncate',
                      unread > 0 && 'font-semibold text-[color:var(--color-text)]'
                    )}
                  >
                    {peer.username}
                  </span>
                  {lastDM && (
                    <span className="text-[10px] text-[color:var(--color-text-mute)] flex-shrink-0">
                      {new Date(lastDM.timestamp).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </span>
                ...
                {lastDM && (
                  <p
                    className={cn(
                      'text-xs truncate',
                      unread > 0
                        ? 'text-[color:var(--color-text-dim)] font-medium'
                        : 'text-[color:var(--color-text-mute)]'
                    )}
                  >
                    {lastDM.authorId === currentUser.id ? 'You: ' : ''}
                    {lastDM.content}
                  </p>
                )}
              </div>
        ...
        {search && sortedFiltered.length > 0 && othersFiltered.length > 0 && (
          <>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-[color:var(--color-text-mute)] px-2 py-2 mt-2">
              All Users
            </p>
            {othersFiltered
              .filter((u) => !sortedFiltered.some((s) => s.id === u.id))
              .map((peer) => (
                <button
                  key={peer.id}
                  onClick={() => {
