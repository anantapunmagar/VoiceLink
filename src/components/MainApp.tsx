...
export function MainApp({ user, onLogout, onUserUpdate }: MainAppProps) {
  ...
  useEffect(() => {
    function refresh() {
      setDmUnread(chatBus.getUnreadDMCount(user.id));
    }
    refresh();
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-[color:var(--color-bg-2)] min-w-0">
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[color:var(--color-border)] bg-[color:var(--color-bg-2)] lg:hidden flex-shrink-0">
          <button
            ...
            className="p-1.5 rounded-lg text-[color:var(--color-text-mute)] hover:bg-[color:var(--color-bg-4)] transition-colors"
          >
            <Menu size={18} />
          </button>
          <button
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-8 gap-5">
      <div className="h-16 w-16 rounded-full flex items-center justify-center bg-[color:var(--color-bg-3)]">
        {isDM ? (
          <MessageSquare size={28} className="text-[color:var(--color-text-mute)]" />
        ) : (
      ...
      <button
        ...
        className="lg:hidden px-4 py-2 rounded-xl bg-[color:var(--color-accent)] text-white text-sm font-medium hover:bg-[color:var(--color-accent-hover)] transition-colors"
      >
        Open Sidebar
      </button>
    </div>
