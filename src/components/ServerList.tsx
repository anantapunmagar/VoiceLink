...
export function ServerList({
  ...
  function handleJoin() {
    ...
    if (!server.members.includes(currentUserId)) {
      server.members.push(currentUserId);
      storage.setServers(allServers);
    }
    ...
    <div className="flex flex-col items-center gap-2 py-3 w-[68px] flex-shrink-0 overflow-y-auto overflow-x-hidden bg-[color:var(--color-bg-0)] h-full">
      ...
      <div className="relative">
        ...
        >
          <Plus
            size={20}
            className="text-[color:var(--color-success)] group-hover:text-white transition-colors"
          />
        </button>
        {showCreate && (
          <Popup title="Create a server" onClose={() => setShowCreate(false)}>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 h-8 text-xs text-[color:var(--color-text-dim)] rounded-lg border border-[color:var(--color-border)] hover:border-[color:var(--color-border-strong)] transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 h-8 text-xs font-semibold bg-[color:var(--color-accent)] text-white rounded-lg disabled:opacity-40 hover:bg-[color:var(--color-accent-hover)] transition-colors"
              >
                Create
              </button>
            </div>
            <button
              ...
              className="w-full text-center text-xs text-[color:var(--color-accent)] hover:underline mt-2"
            >
              Join with invite code instead
            </button>
          </Popup>
        )}
        {showJoin && (
          ...
          >
            <input
              ...
              placeholder="Invite code (e.g. AB12CDE)"
              autoFocus
              maxLength={16}
            ...
            />
            {joinError && (
              <p className="text-xs text-[color:var(--color-danger)] mb-2">{joinError}</p>
            )}
            <div className="flex gap-2">
              <button
                onClick={() => {
  ...
  return (
    ...
    >
      <button
        onClick={onClick}
        className={cn(
          'relative h-12 w-12 rounded-2xl flex items-center justify-center font-bold text-sm transition-all duration-200 focus-ring text-white',
      ...
      >
        {icon ?? initials}
        {active && (
          <span className="absolute -left-3 top-1/2 -translate-y-1/2 h-5 w-1.5 bg-white rounded-full" />
        )}
        {active && (
          <span
            className="absolute inset-0 rounded-2xl ring-2 ring-white/20"
            style={{ borderRadius: active ? '12px' : '16px' }}
          />
        )}
      </button>
      {badge != null && badge > 0 && (
