...
const bus = (() => {
  ...
  const reactionListeners = new Set<
    (p: { msgId: string; key: string; userId: string; add: boolean }) => void
  >();
  const typingListeners = new Set<
    (p: { userId: string; channelId: string; typing: boolean }) => void
  >();
  ...
  function dispatch(signal: ChatSignal) {
    ...
    switch (signal.type) {
      case 'message':
        msgListeners.forEach((f) => f(signal.payload as Message));
        break;
      case 'message-edit':
        editListeners.forEach((f) => f(signal.payload as Message));
        break;
      case 'message-delete':
        deleteListeners.forEach((f) => f(signal.payload as string));
        break;
      case 'reaction':
        reactionListeners.forEach((f) =>
          f(signal.payload as { msgId: string; key: string; userId: string; add: boolean })
        );
        break;
      case 'typing':
        typingListeners.forEach((f) =>
          f(signal.payload as { userId: string; channelId: string; typing: boolean })
        );
        break;
      case 'dm':
        dmListeners.forEach((f) => f(signal.payload as DirectMessage));
        break;
      case 'call-invite':
      case 'call-answer':
      case 'call-reject':
  ...
  function sendTyping(userId: string, channelId: string, typing: boolean) {
    post({
      type: 'typing',
      payload: { userId, channelId, typing },
      senderId: userId,
      timestamp: Date.now(),
    });
  }
  ...
  // ---- Subscriptions ----
  function onMessage(fn: (msg: Message) => void) {
    getChannel();
    msgListeners.add(fn);
    return () => {
  ...
  function onEdit(fn: (msg: Message) => void) {
    getChannel();
  ...
  function onDelete(fn: (id: string) => void) {
    getChannel();
  ...
  function onReaction(
    fn: (p: { msgId: string; key: string; userId: string; add: boolean }) => void
  ) {
    getChannel();
    reactionListeners.add(fn);
    return () => {
      reactionListeners.delete(fn);
    };
  }
  function onTyping(fn: (p: { userId: string; channelId: string; typing: boolean }) => void) {
    getChannel();
    typingListeners.add(fn);
    return () => {
      typingListeners.delete(fn);
    };
  }
  function onDM(fn: (dm: DirectMessage) => void) {
    getChannel();
    dmListeners.add(fn);
    return () => {
  ...
  function onCallSignal(fn: (s: ChatSignal) => void) {
    getChannel();
